...
export function PassportApiTool({
  ...
  const fakeUsersIntervalRef = useRef<ReturnType<typeof setInterval> | null>(
    null,
  );
      ...
      try {
        ...
        const sorted =
          list && list.length > 0
            ...
            : FALLBACK_COUNTRIES;
        setCountries(sorted);
        const resolvedCode = sorted.find(
          (c) => c.country_code === defaultCountryCode,
        )
          ? defaultCountryCode
          : sorted[0].country_code;
        ...
        const active = FALLBACK_COUNTRIES.find(
          (c) => c.country_code === defaultCountryCode,
        );
        if (active) {
          setSearchQuery(active.country_name);
  ...
  const handleProcessImage = async () => {
    ...
    try {
      const response = await processExternalPhoto(
        selectedFile,
        selectedCountryCode,
      );
      clearProcessingInterval();
      ...
      setTimeout(() => {
        ...
        setStage("result");
        toast.success(
          "AI processing complete — photo is government-compliant!",
        );
      }, 400);
    } catch (error: any) {
      clearProcessingInterval();
      ...
      const errorMessage =
        error?.message ||
        "Processing failed. Ensure your face is fully visible and well-lit.";
  ...
  const handleDownloadPrintSheet = () => {
    if (result?.print_sheet_url) {
      handleDownload(
        ...
        `${selectedCountryCode}_print_sheet_4x6.jpg`,
      );
    }
  };
    ...
    <div className="min-h-screen  font-sans">
      <div className="max-w-2xl mx-auto px-1 py-6 space-y-4 pb-8">
          ...
          <div className="flex items-start gap-4">
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-muted-foreground mt-1 leading-relaxed">
                Government-compliant photos for 150+ countries — background
                removed, auto-cropped, print-ready.
              </p>
              ...
              <div className="flex flex-wrap gap-2 mt-3 ">
                {["Background Removed", "Auto-Crop", "Print-Ready"].map((t) => (
                  <span
                    key={t}
                    className="inline-flex items-center gap-1 text-[11px] font-semibold text-green-700 dark:text-green-400 bg-green-50 dark:bg-green-900/30 px-2 py-0.5 rounded-full border border-green-100 dark:border-green-900"
                  >
        ...
        {stage === "idle" && (
          ...
          >
            <div
              className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-4 transition-colors duration-200 ${isDragOver ? "bg-primary/10" : "bg-muted"}`}
            >
              <ImageIcon
                className={`w-7 h-7 transition-colors duration-200 ${isDragOver ? "text-primary" : "text-muted-foreground"}`}
              />
            </div>
          ...
          <>
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 bg-red-600 hover:bg-red-700 active:scale-[0.98] text-white font-bold rounded-2xl py-4 text-sm flex items-center justify-center gap-2 transition-all shadow-lg shadow-red-600/20"
              >
        ...
        {stage === "processing" && (
          <div className="bg-card rounded-2xl border border-border shadow-clean-sm p-8 flex flex-col items-center text-center gap-6">
            ...
            <div>
              ...
              <p className="text-sm text-primary font-medium mt-1 min-h-[20px]">
                {processingMsg}
              </p>
            </div>
            ...
            <div className="flex flex-wrap justify-center gap-2 text-[11px] text-muted-foreground">
              {[
                ...
                <span
                  key={s}
                  className={`px-2.5 py-1 rounded-full border ${processingProgress > i * 30 + 10 ? "border-primary/30 bg-primary/10 text-primary font-semibold" : "border-input bg-muted"}`}
                >
                  {s}
                </span>
              ))}
          ...
          <>
              ...
              {showDownloadOptions && !isPaid ? (
                    ...
                    {/* <button
                      onClick={() => {
                        if (result) {
                          startFreeDownload(
                            result.image_url,
                            `${selectedCountryCode}_passport_photo.jpg`,
                          );
                          setShowDownloadOptions(false);
                        }
                      }}
                ...
                <div className="mt-4 p-4 bg-primary/5 border border-primary/30 rounded-xl">
                  ...
                  <button
                    onClick={() => {
                      console.log(
                        "[PassportApiTool] Pay clicked, priceInfo:",
                        priceInfo,
                      );
                ...
                <button
                  ...
                  className="w-full mt-5 bg-red-600 hover:bg-red-700 disabled:opacity-50 active:scale-[0.98] text-white font-bold rounded-xl py-3.5 text-sm flex items-center justify-center gap-2 transition-all"
                >
            ...
            {result.print_sheet_url && !isPaid && waitCountdown === null ? (
              ...
              >
                ...
                {isDownloading
                  ? "Preparing..."
                  : "Download 4×6 Print Sheet (Free - 3 min wait)"}
              </button>
            ) : result.print_sheet_url &&
              !isPaid &&
              waitCountdown === null &&
              ...
              >
                ...
                {isDownloading
                  ? "Preparing..."
                  : `Unlock Print Sheet · ${priceInfo.formatted}`}
              </button>
            ) : null}
