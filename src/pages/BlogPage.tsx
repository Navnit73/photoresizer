...
export default function BlogPage() {
  ...
  const getCategoryIcon = () => {
    if (post.category === "Official IDs")
      return (
        <Camera className="w-6 h-6 mr-3 text-blue-600 dark:text-blue-400" />
      );
    if (post.category === "Technical Tools")
      return (
        <FileImage className="w-6 h-6 mr-3 text-blue-600 dark:text-blue-400" />
      );
    if (post.category === "Exam Guides")
      return (
        <FileIcon className="w-6 h-6 mr-3 text-blue-600 dark:text-blue-400" />
      );
    return (
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white dark:bg-slate-950 font-sans selection:bg-blue-100 selection:text-blue-900">
      ...
      <main className="flex-1 w-full max-w-6xl mx-auto px-4 py-8 md:py-12">
        <Button
          onClick={() => navigate("/")}
          className="w-full font-extrabold text-xl bg-purple-600 hover:bg-purple-500 mb-4 text-white h-16 shadow-[0_0_20px_rgba(220,38,38,0.4)] transition-all active:scale-95"
          size="lg"
        >
          {post.category === "Technical Tools"
            ? "Compress Now"
            : post.category === "Exam Guides"
              ? "Upload & Resize"
              : "Upload & Resize"}
        </Button>
        {/* ================= BREADCRUMB ================= */}
          ...
          <article className="min-w-0">
            ...
            <header className="mb-10">
              ...
              <div className="flex flex-wrap items-center gap-4 text-sm text-slate-500 dark:text-slate-400 mb-8 pb-8 border-b border-slate-100 dark:border-slate-800">
                <div className="flex items-center">
                  <Badge
                    variant="outline"
                    className="rounded-md border-blue-200 bg-blue-50 text-blue-700 dark:bg-blue-900/20 dark:text-blue-300 dark:border-blue-800 px-2 py-0.5"
                  >
                    {post.category}
                  </Badge>
                </div>
                ...
                <div className="flex items-center text-slate-400">
                  &bull; {post.content.split(/\s+/).length > 300 ? "5" : "3"}{" "}
                  min read
                </div>
              </div>
            ...
            <div className="lg:hidden bg-slate-900 text-white rounded-xl p-6 mb-10 shadow-lg">
              <h3 className="font-bold text-lg mb-2">
                {post.category === "Official IDs"
                  ? `Resize ${post.title.split(" ").slice(0, 3).join(" ")}`
                  : post.category === "Exam Guides"
              ...
              <p className="text-slate-300 text-sm mb-4">
                ...
                post.specifications?.dimensions !== "Any"
              ...
              <div className="mb-12">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    post.specifications.dimensions !== "Any" && (
                      <div className="bg-slate-50 dark:bg-slate-900/50 rounded-xl p-5 border border-slate-100 dark:border-slate-800 flex flex-col items-start hover:border-blue-200 transition-colors group">
                        ...
                        {post.slug.includes("passport") && (
                          <span className="text-xs text-blue-600 dark:text-blue-400 mt-2 font-medium">
                            Indian Standard
                          </span>
                        )}
                      </div>
                    ...
                    post.specifications.fileSize !== "-" && (
                      <div className="bg-slate-50 dark:bg-slate-900/50 rounded-xl p-5 border border-slate-100 dark:border-slate-800 flex flex-col items-start hover:border-blue-200 transition-colors group">
                        ...
                        {post.specifications.fileSize.includes("10 KB") && (
                          <span className="text-xs text-amber-600 dark:text-amber-400 mt-2 font-medium">
                            Strict limit
                          </span>
                        )}
                      </div>
                  ...
                  {post.specifications.format &&
                    post.specifications.format !== "-" &&
                    post.specifications.format !== "Convert to .jpg" && (
                      <div className="bg-slate-50 dark:bg-slate-900/50 rounded-xl p-5 border border-slate-100 dark:border-slate-800 flex flex-col items-start hover:border-blue-200 transition-colors group">
                        <span className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-2 flex items-center w-full">
                        ...
                        {post.specifications.format.includes("JPG") && (
                          <span className="text-xs text-green-600 dark:text-green-400 mt-2 font-medium">
                            Most compatible
                          </span>
                        )}
                      </div>
                  ...
                  {post.slug === "convert-jpeg-to-jpg-online" && (
                    <div className="bg-slate-50 dark:bg-slate-900/50 rounded-xl p-5 border border-slate-100 dark:border-slate-800 flex flex-col items-start hover:border-blue-200 transition-colors md:col-span-3">
                      ...
                      <span className="text-xs text-blue-600 dark:text-blue-400 mt-2">
                        Original quality preserved • No data loss • Instant
                      </span>
                    </div>
                  )}
                  ...
                  {post.category === "Exam Guides" &&
                    post.specifications.totalExams && (
                      <>
                        ...
                        <div className="bg-slate-50 dark:bg-slate-900/50 rounded-xl p-5 border border-slate-100 dark:border-slate-800 flex flex-col items-start">
                          ...
                          <div className="font-mono text-lg font-bold text-slate-900 dark:text-white">
                            {post.specifications.totalVacancies}
                          </div>
                        </div>
                        <div className="bg-slate-50 dark:bg-slate-900/50 rounded-xl p-5 border border-slate-100 dark:border-slate-800 flex flex-col items-start">
                          <span className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-2">
                          ...
                          <div className="font-mono text-lg font-bold text-green-600 dark:text-green-400">
                            {post.specifications.applicationStatus}
                          </div>
                        </div>
                      </>
                    )}
                ...
                >
                  {post.faq.map((item, index) => (
                    <AccordionItem
                      key={index}
                      value={`item-${index}`}
                      className="border rounded-xl px-2 border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 data-[state=open]:bg-white dark:data-[state=open]:bg-slate-900 data-[state=open]:shadow-md transition-all duration-200"
                    >
            ...
            <div className="bg-slate-900 text-white p-8 rounded-2xl shadow-xl relative overflow-hidden group">
              ...
              <p className="text-slate-300 text-sm mb-6 relative z-10">
                ...
                post.specifications?.dimensions !== "Any"
                  ...
                  : post.category === "Exam Guides"
                    ? "Get exam-ready photos instantly"
                ...
                {post.specifications?.dimensions &&
                  ...
                  post.specifications.dimensions !== "Any" && (
                    <div className="flex items-center text-sm text-slate-200">
                      <div className="w-6 h-6 rounded-full bg-green-500/20 flex items-center justify-center mr-3 text-green-400 font-bold">
                        ✓
                      </div>
                      {post.specifications.dimensions}
                    </div>
              ...
              <Button
                onClick={() => navigate("/")}
                className="w-full font-extrabold text-xl bg-red-600 hover:bg-red-500 text-white h-16 shadow-[0_0_20px_rgba(220,38,38,0.4)] transition-all active:scale-95"
                size="lg"
              >
                {post.category === "Technical Tools"
                  ? "Compress Now"
